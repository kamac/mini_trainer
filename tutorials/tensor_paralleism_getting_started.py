...
if local_rank == 0:
    ...
    for convo in ds["train"]["conversations"]:
        ...
        for turn in convo:
            ...
            if len(tmp) and tmp[-1]["role"] == role:
                tmp[-1]["content"] += "\n\n" + turn["value"]
            else:
                tmp.append({"role": role, "content": turn["value"]})
...
if local_rank == 0 or local_rank == 1:
    example = to_input_example(dataset[0])
elif local_rank == 2 or local_rank == 3:
    example = to_input_example(dataset[1])
elif local_rank == 4 or local_rank == 5:
    example = to_input_example(dataset[2])
elif local_rank == 6 or local_rank == 7:
    example = to_input_example(dataset[3])
...
with torch.no_grad():
    ...
    print(
        f"\033[38;5;208m DP - rank {local_rank} | loss sum {reduced_loss.item() / 2}\033[0m"
    )  # divide by 2 because we are running an example twice.
...
for x in batch_examples:
    total_loss += tp_model(**x).loss.float().sum()
with torch.no_grad():
    reduced_loss = total_loss.clone()
    torch.distributed.all_reduce(reduced_loss, op=torch.distributed.ReduceOp.SUM)
    print(
        f"\033[38;5;208m TP - rank {local_rank} | loss sum {reduced_loss.item() / 8}\033[0m"
    )  # divide by 8 because we are using 8 GPUs.
...
if local_rank == 0:
    print(f"fsdp + tp mesh: {world_mesh}")
...
with torch.no_grad():
    ...
    torch.distributed.all_reduce(reduced_loss, op=torch.distributed.ReduceOp.SUM)
    print(
        f"\033[38;5;208m FSDP + TP - rank {local_rank} | loss sum {reduced_loss.item() / 2}\033[0m"
    )  # divide by 2 because we are adding loss twice per example, due to TP.
    # >>> FSDP + TP - rank 0 | loss sum 262.120819091796 ... and same for other ranks
loss.backward()
optimizer.step()
fsdp_tp_model.zero_grad()
# delete the model from memory and clear the cache
del fsdp_tp_model, optimizer
torch.cuda.empty_cache()
