...
def test_conversion_accuracy():
    ...
    try:
        ...
        try:
            ...
            for orig_name, orig_tensor in orig_expert_params.items():
                if orig_name in conv_expert_params:
                    ...
                    if torch.equal(orig_tensor, conv_tensor):
                        ...
                        print(
                            f"     ⚠️  Partial match {orig_name}: {diff_count}/{total_elements} different ({diff_percent:.1f}%)"
                        )
            ...
            total_comparisons = len(orig_expert_params) + len(
                [n for n in conv_expert_params if n not in orig_expert_params]
            )
